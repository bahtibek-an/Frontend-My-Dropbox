...
const Folder = ({ folder }) => {
  ...
  return (
    <>
      <Card
        variant="success"
        className="card-body text-dark text-dark-none">
          <div className="d-flex justify-content-between align-items-center me-3" style={{marginTop: "-25px", }}>
            <FontAwesomeIcon icon={faFolder} className="text-primary w-50 h-50" />
            <div class="dropdown dropstart">
              <span class="" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                <FontAwesomeIcon icon={faEllipsisV} />
              </span>
              <ul class="dropdown-menu dropdown-menu-end dropdown-menu-lg-start">
                <button onClick={() => deleteDocument(folder.id)} className="btn btn-danger w-90 mb-1 m-2 p-2">Delete <FontAwesomeIcon icon={faTrash} /></button>
              </ul>
            </div>
          </div>
        <Link to={{ pathname: `/folder/${folder.id}`, state: { folder } }} className="text-dark">{folder.name}</Link>
      </Card>
    </>
  );
};
